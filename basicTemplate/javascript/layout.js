...
/*function addPrint(layers) {
    ...
    var templates = dojo.map(configOptions.printlayouts, function (layout) {
        ...
        layout.preserveScale = true;

        return layout;
    });

    updatePrint(layers,templates);
}
...
function PrintStart() {
    ...
    function addToMap(evt) {
        ...
        var gsvcurl= configOptions.helperServices.geometry.url;
        var gsvc = new esri.tasks.GeometryService(gsvcurl);
        var outSR = new esri.SpatialReference(102100);
        gsvc.project([extent],outSR,function(projectedPy)
        {

           var py = projectedPy[0];
           var Xwidth=py.xmax-py.xmin;
           var Yheight=py.ymax-py.ymin;

            if (dijit.byId('PrintPanel').get('value') == "L1") {
                var printurl= configOptions.helperServices.printTask.url;
                var printTask = new esri.tasks.PrintTask(printurl);


                var template = new esri.tasks.PrintTemplate();
                template.outScale = 4508.935440959931;
                var ww=CalculateWH(Xwidth,template.outScale);
                var hh=CalculateWH(Yheight,template.outScale);
            ...
            if (dijit.byId('PrintPanel').get('value') == "L2") {
                ...
                var template = new esri.tasks.PrintTemplate();
                template.outScale = 9017.870881919862;
                var ww=CalculateWH(Xwidth,template.outScale);
                var hh=CalculateWH(Yheight,template.outScale);
                template.exportOptions = {
                    width: ww,
            ...
            if (dijit.byId('PrintPanel').get('value') == "L3") {
                ...
                var template = new esri.tasks.PrintTemplate();
                template.outScale =18035.741763839724;
                var ww=CalculateWH(Xwidth,template.outScale);
                var hh=CalculateWH(Yheight,template.outScale);
                template.exportOptions = {
                    width: ww,
...
function CalculateWH(x,scale){
    
    var touying=x*(1/scale)*100;
    var inch=touying*0.39*96;
    return inch;
